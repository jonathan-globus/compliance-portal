...
| :--- | :--- | :--- |
| **Mandatory Encryption** | All data transfers to and from High Assurance collections are encrypted in transit. Encryption cannot be disabled by users or administrators. TLS 1.2 is enforced for both the data and control channels. | Enforced by Globus services |
| **Strong Authentication** | Access requires authentication through approved identity providers. Institutions may require MFA or additional identity assurance. | Enforced by Globus services and institutional identity policies and IdP configuration |
| **Session and Token Restrictions** | High Assurance collections use shorter session lifetimes and stricter token scopes to reduce risk of unauthorized access. | Enforced by Globus services |
| **Granular Access Controls** | Institutions define which users or groups may access sensitive collections. Access can be limited to specific operations or paths. | Configured and managed by institution |
| **Audit Logging** | Detailed logs record access, transfers, and administrative actions. Logs support institutional monitoring and compliance review. | Enforced by Globus and institution configuration settings |
| **Administrative Policy Controls** | Institutions can apply additional restrictions, such as limiting sharing, requiring specific identity providers, or restricting external access. | Enforced by institution configuration settings |
| **Secure Endpoint Configuration** | High Assurance endpoints must be configured to meet security requirements for sensitive data environments. | Enforced by institution configuration settings |
